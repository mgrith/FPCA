...
% Input:      
         %N         - Number of curves
         %k         - Random State
         %T         - Number of obervations per curve
...
for i=2:(N+1) 
    ...
    for j=1:T  
        ...
        for l=1:3
            qu(l,:) =1./(x*sqrt(2*pi*s(l)^2*tau)).*exp(-1/2*((log(x)- (m(l)-s(l)^2/2)*tau)/(s(l)*sqrt(tau))).^2);  %density components of the mixture -------added space/aligned
